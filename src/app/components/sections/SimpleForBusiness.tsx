...
import { Rocket, MousePointerClick, Boxes, MessageCircle, Bot, Zap, Sparkles, ArrowRight } from "lucide-react";
        ...
        >
          {items.map((item, index) => (
            ...
            >
              ...
              {/* Основная карточка */}
              <div className={`relative rounded-2xl ${item.gradient} border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-md p-8 shadow-sm hover:shadow-2xl transition-all duration-500 group-hover:border-slate-300/70 dark:group-hover:border-slate-600/70`}>
                
                {/* Верхний акцент */}
                <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${item.accent} rounded-t-2xl`} />
